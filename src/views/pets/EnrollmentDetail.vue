<template>
          ...
          <div class="space-y-2 text-sm">
            ...
            <div class="pt-2">
              ...
              <div>
                ...
                <span class="ml-2 text-green-600 font-semibold">{{ formatCOP(enrollment.plan.price) }}</span>
              </div>
            </div>
            ...
            <div class="mt-5 pt-3 border-top-1 surface-border">
              ...
              <div>
                ...
                <span class="ml-2" :class="enrollment.is_active ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'">
  ...
  <EditPlanModal
    ...
    :currentPlanId="enrollment.plan.id"
  ...
  methods: {
    ...
    async loadDetail() {
      ...
      try {
        ...
        this.toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar la matrícula', life: 3000 })
      } finally {
        this.loading = false
      }
    },
    editEnrollment() { this.showPlanEditor = true },
    onUpdated() {
      this.toast.add({ severity: 'success', summary: 'Actualizada', detail: 'Matrícula actualizada correctamente', life: 2500 })
      this.showPlanEditor = false
      this.loadDetail()
    },
    onError(msg) {
      this.toast.add({ severity: 'error', summary: 'Error', detail: msg || 'No se pudo completar la acción', life: 3000 })
    },
    confirmDelete() {
      this.confirm.require({
        header: 'Confirmar cancelación',
        message: 'Al cancelar la matrícula no habrá reembolso y, si desea continuar en el programa, deberá adquirir un nuevo plan.',
    ...
    async deleteEnrollment() {
      ...
      try {
        ...
        this.toast.add({ severity: 'success', summary: 'Eliminada', detail: 'Matrícula cancelada correctamente', life: 2500 })
        this.$router.push({ path: '/admin/pets' })
      } catch (e) {
        console.error('Error cancelando matrícula:', e)
        this.toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cancelar la matrícula', life: 3000 })
      } finally {
        this.deleting = false
      }
    }
